...
UINT8           aEthernetReceiveBuffer[ETHERNET_MAX_FRAMEBUF_LEN];     /**< \brief this buffer is used to receive Ethernet frames */
#endif
    ...
    if ( mbxSize >= ETHERCAT_EOE_HEADER_LEN)
    {
        ...
        case EOE_TYPE_FRAME_FRAG:
            ...
            if ( u8ReceiveFragmentNo == 0 )
            {
                ...
                /*ECATCHANGE_START(V5.13) EOE4*/
                if ( u16EthernetReceiveSize > ETHERNET_MAX_FRAMEBUF_LEN)
                /*ECATCHANGE_END(V5.13) EOE4*/
                {
