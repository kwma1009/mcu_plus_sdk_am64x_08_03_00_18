...
UINT8 MailboxServiceInd(TMBX MBXMEM *pMbx)
{
    ...
    case MBX_TYPE_EOE:
#if !MAILBOX_QUEUE
        /* Reset the pending response flag (EoE datagrams does not require a response)*/
        bPendingMbxResponse = FALSE;
#endif
...
/*ECATCHANGE_START(V5.13) TEST3*/
if (IS_TEST_ACTIVE(SendArpOnUnsupportedMailboxProtocolRequest))
{
    UINT16 u16result = 0;
    ...
    u16result = EOE_SendFrameRequest((UINT16 MBXMEM *) pSendFrame, ARP_IP_HEADER_LEN + ETHERNET_FRAME_LEN);

    if (u16result == 0)
    {
        INC_TEST_CNT(SendArpOnUnsupportedMailboxProtocolRequest);
    }
}
/*ECATCHANGE_END(V5.13) TEST3*/
...
#if TEST_APPLICATION && EOE_SUPPORTED
    if (i8EoeSendPending == 2)//send ethernet frame data was written
    {
        i8EoeSendPending = 1; //write ethernet data on next mainloop
    }
#endif
...
/*ECATCHANGE_END(V5.13) MBX3*/
      ...
      if ( u8MailboxSendReqStored )
    {
        ...
        if ( u8MailboxSendReqStored & SOE_SERVICE )
        {
            ...
            /* call SoE function that will send the stored SoE service */
...
UINT8 MBX_MailboxSendReq( TMBX MBXMEM * pMbx, UINT8 flags )
{
    ...
    else
    {
        ...
        if (IS_TEST_ACTIVE(Mbx_NoMbxCntIncrement))
        {
            u8MbxReadCounter = 0;
            INC_TEST_CNT(Mbx_NoMbxCntIncrement);
        }
