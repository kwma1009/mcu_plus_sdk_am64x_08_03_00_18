...
#if USE_SINGLE_PDO_MAPPING_ENTRY_DESCR
        || (IS_RX_PDO(pObjEntry->Index) || IS_TX_PDO(pObjEntry->Index))
#endif
#if TEST_APPLICATION
        || IS_TEST_OBJECT(index)
        || (index == 0x8000)    //Slave config object
#endif
            )
        {
#if TEST_APPLICATION
            if(index == 0x8000)
            {
                size = ((SlaveConfigObjSubindex0 * pObjEntry->pEntryDesc[1].BitLength) +7)>>3;
...
#endif // COMPLETE_ACCESS_SUPPORTED
    {
        if ( objCode == OBJCODE_VAR )
        {
#if TEST_APPLICATION
            ...
            if(index == 0x300C)
            {
                return (BIT2BYTE(OBJ_300C_ENTRY_BITSIZE));
            }
            else
#endif
#endif
        ...
        {
#if DIAGNOSIS_SUPPORTED
            if(index == 0x10F3 && subindex >= 6)
            {
                return aDiagMessages[(subindex-6)].u16MessageLength;
            }
            else
#endif
            {
...
#if TEST_APPLICATION
    if(IS_TEST_ACTIVE(SDOInfo_DummyOD_Test))
    {
        INC_TEST_CNT(SDOInfo_DummyOD_Test);
        return DUMMY_OD_LENGTH;
    }
#endif
...
/* get the information of ObjCode and MaxSubindex in local variables to support different types of microcontroller */
    ...
    else
    {
        ...
        {
#if TEST_APPLICATION
            if(IS_TEST_OBJECT(index) && subindex > 0)
            {
                TTESTOBJ OBJMEM *pTestObj = (TTESTOBJ OBJMEM *)pObjEntry->pVarPtr;

                strSize = (UINT16) OBJSTRLEN((OBJCONST CHAR OBJMEM *) pTestObj->paEntries[(subindex - 1)]->pName);
                if ( pData && strSize )
                {
                    OBJTOMBXSTRCPY( pData, pTestObj->paEntries[(subindex - 1)]->pName, strSize );
                }
            }
            else if(index == 0x8000) //slave configuration object
            {
               strSize = (UINT16) OBJSTRLEN((OBJCONST CHAR OBJMEM *) aSlaveConfig[(subindex - 1)].pName);

               if ( pData && strSize )
               {
                    OBJTOMBXSTRCPY( pData, aSlaveConfig[(subindex-1)].pName, strSize );
               }
            }
            else
#endif
...
OBJCONST TSDOINFOENTRYDESC OBJMEM * OBJ_GetEntryDesc(OBJCONST TOBJECT OBJMEM * pObjEntry, UINT8 Subindex)
{
...
#endif
#if TEST_APPLICATION
            /*also Test objects have the same entry description even if the object type is record */
            || (IS_TEST_OBJECT(pObjEntry->Index))
            || (pObjEntry->Index == 0x8000) //Slave config object
#endif
        )
    {
        /* object is an array */
    ...
    {
#if DIAGNOSIS_SUPPORTED
        //Diagnosis message is request
        if((pObjEntry->Index == 0x10F3) && Subindex > 5)
        {
            MessageEntryDesc.BitLength  = BYTE2BIT(aDiagMessages[(Subindex-6)].u16MessageLength);
            pEntry = &MessageEntryDesc;
        }
        else
#endif
        {
...
#if TEST_APPLICATION
        if(IS_TEST_ACTIVE(SDOFullObjectCompleteAccess_Test) && bCompleteAccess)
        {
            //test behaviour, return all object data not only range configured via SI0
            maxSubindex = (pObjEntry->ObjDesc.ObjFlags & OBJFLAGS_MAXSUBINDEXMASK) >> OBJFLAGS_MAXSUBINDEXSHIFT;
            INC_TEST_CNT(SDOFullObjectCompleteAccess_Test);
        }

        //the test object 0x300B is not supported on platforms which does not support 8bit datatypes
        if (index == 0x300B && (SIZEOF(UINT8) > 1) && ((bCompleteAccess == TRUE) || (subindex > 0)))
        {
...
//if 0x3004.14 is accessed via single access the operation shall be blocked
            if (index == 0x3004 && subindex == 14 && bCompleteAccess == FALSE)
            {
                return ABORTIDX_DATA_CANNOT_BE_ACCESSED_BECAUSE_OF_LOCAL_CONTROL;
            }
#endif
...
#if MBX_16BIT_ACCESS  && !MEMORY_UNIT_16BIT
...
/*ECATCHANGE_START(V5.13) */
#if MEMORY_UNIT_16BIT
            pData[0] = (0x00FF & pVarPtr[1]) << 8 | (0x00FF & pVarPtr[0]);
            pData[1] = (0x00FF & pVarPtr[3]) << 8 | (0x00FF & pVarPtr[2]);
...
#if (BIG_ENDIAN_16BIT || BIG_ENDIAN_FORMAT)
            pData[0] = SWAPWORD(pVarPtr[1]);
            pData[1] = SWAPWORD(pVarPtr[0]);
#else
            pData[0] = pVarPtr[0];
            pData[1] = pVarPtr[1];
#endif
                ...
                if (0 != (((UINT8) ((pEntry->ObjAccess & ACCESS_READ)<<1)) & (nAlStatus & STATE_MASK)) )
                {
                        ...
                        || (bCompleteAccess && i >= subindex) )       /* complete access and entry should be read */
                    {
                        ...
                        if ( i == 0 && objCode != OBJCODE_VAR )
                        {
                            ...
                            if(IS_TEST_ACTIVE(SDOFullObjectCompleteAccess_Test) && bCompleteAccess)
                            {
                                pData[0] = SWAPWORD((*((UINT16 *) (pObjEntry->pVarPtr))) & 0x00FF);
                            }
                            else
#endif
                            {
...
#if BIG_ENDIAN_16BIT || BIG_ENDIAN_FORMAT
                                pData[0] = SWAPWORD(pVarPtr[3]);
                                pData[1] = SWAPWORD(pVarPtr[2]);
                                pData[2] = SWAPWORD(pVarPtr[1]);
                                pData[3] = SWAPWORD(pVarPtr[0]);
#else
                                pData[0] = pVarPtr[0];
                                pData[1] = pVarPtr[1];
                                pData[2] = pVarPtr[2];
...
/*ECATCHANGE_END(V5.13) TEST7*/
                        ...
                        case    DEFTYPE_UNSIGNED64:
                            ...
                            /* in this example the objects are defined in that way,
                               that the 64 bit type are always starting at an exact WORD offset */
#if BIG_ENDIAN_16BIT || BIG_ENDIAN_FORMAT
                            pVarPtr[0] = SWAPWORD(pData[3]);
                            pVarPtr[1] = SWAPWORD(pData[2]);
                            pVarPtr[2] = SWAPWORD(pData[1]);
                            pVarPtr[3] = SWAPWORD(pData[0]);
#else
                            pVarPtr[0] = pData[0];
                            pVarPtr[1] = pData[1];
                            pVarPtr[2] = pData[2];
...
#if STORE_BACKUP_PARAMETER_IMMEDIATELY && BACKUP_PARAMETER_SUPPORTED
