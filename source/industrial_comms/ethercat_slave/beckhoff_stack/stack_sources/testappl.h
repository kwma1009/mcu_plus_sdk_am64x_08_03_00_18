...
/* CTT TestID Defines*/
#define SET_CTT_TEST_REQUEST (UINT16)0x002F  /**< \brief AL Control Value to set the CTT Test IDs (TestID,GroupId,CaseID) (object id is written to "ESC_CTT_TESTID_REGISTER"*/

#define SET_CTT_TEST_REQUEST_PENDING(alcontrol) (((nAlStatus & STATE_MASK) == STATE_INIT) && ((((UINT16)alcontrol) & SET_CTT_TEST_REQUEST) == SET_CTT_TEST_REQUEST)) /**< \brief test object shall only be set in Init to 0x2F transition*/

#define ESC_CTT_TESTID_REGISTER     0x0600 /**< \brief CTT Test ID Register*/
...
#define IS_COE_COMMAND(pMbx) ((pMbx != NULL) && (((((TMBX MBXMEM *)pMbx)->MbxHeader.Flags[MBX_OFFS_TYPE] & MBX_MASK_TYPE) >> MBX_SHIFT_TYPE) == MBX_TYPE_COE)) /**< \brief Checks if the memory contains a CoE command*/

#define IS_SDO_COMMAND(pCoE) (IS_COE_COMMAND(pCoE) && (((((TCOEMBX MBXMEM *) pMbx)->CoeHeader & COEHEADER_COESERVICEMASK) >> COEHEADER_COESERVICESHIFT) == COESERVICE_SDOREQUEST)) /**< \brief Checks if the memory contains a SDO command*/
...
#define SET_TEST_REQUEST (UINT16)0x001F  /**< \brief AL Control Value to active a test object (object id is written to "ESC_TESTID_REGISTER"*/

#define SET_TEST_REQUEST_PENDING(alcontrol) (((nAlStatus & STATE_MASK) == STATE_INIT) && ((((UINT16)alcontrol) & SET_TEST_REQUEST) == SET_TEST_REQUEST)) /**< \brief test object shall only be set in Init to 0x1F transition*/

#define ESC_TESTID_REGISTER     0x0800 /**< \brief Test ID Register*/
...
#endif //#if !TEST_APPLICATION_REDUCED_MEMORY
...
/** \brief Test : Pending PS transition with error*/
PROTO TTESTENTRY ESM0_PENDING_PS_TRANSITION_WITH_ERROR
#ifdef _TESTAPPL_
= { "Pending PS transition with error on first try",0,0 }
#endif
;
...
#ifdef _TESTAPPL_
= {0x06, {&ESM0_IP0x16,&ESM0_PS0x1D,&ESM0_PS0x1E,&ESM0_PS_NO_INPUT_UNLOCK,&ESM0_PENDING_TRANSITIONS,&ESM0_PENDING_PS_TRANSITION_WITH_ERROR }}
#endif
;
...
/*---------------------------------------------
-    Objects 0x2010 - 0x2001F : Mailbox behavior
-----------------------------------------------*/
...
/** \brief Test : Do not increment mailbox counter*/
PROTO TTESTENTRY Mbx_NoMbxCntIncrement
#ifdef _TESTAPPL_
= { "Do not increment the mailbox counter",0,0 }
#endif
;

/** \brief Test : Ignore mailbox repeat requests*/
PROTO TTESTENTRY Mbx_IgnoreMbxRepeatRequest
#ifdef _TESTAPPL_
= { "IgnoreMailboxRepeatRequest",0,0 }
#endif
;
...
#ifdef _TESTAPPL_
= {0x04, {&Mbx_MasterMbxCntIncByOne,&Mbx_AlternatingMbxCntChange,&Mbx_NoMbxCntIncrement,&Mbx_IgnoreMbxRepeatRequest}}
#endif
;
#ifdef _OBJD_
...
/*---------------------------------------------
-    Objects 0x2020 - 0x2002F : CoE behavior
-----------------------------------------------*/
...
#ifdef _TESTAPPL_
    ={"Send an emergency on any SDO request in SafeOP and PreOP",0,0}
#endif
;
...
#ifdef _TESTAPPL_
= { 0x7, { &CoE0_SegSdoByte, &CoE0_ESM_Diag, &CoE0_Diag_Test, &SDOInfo_DummyOD_Test, &SDOFullObjectCompleteAccess_Test, &EmergencyOnSdoRequest_Test, &DelaySdoResponse0x3006 } }
#endif
;
#ifdef _OBJD_
...
/*---------------------------------------------
-    Objects 0x2030 - 0x2003F : FoE behavior
-----------------------------------------------*/
...
/*---------------------------------------------
-    Objects 0x2040 - 0x2004F : EoE behavior
-----------------------------------------------*/

/** \brief Test : Send an EoE ARP on all SDO access (upload/info/download) on 0x1018*/
PROTO TTESTENTRY SendEoEOn0x1018Access
#ifdef _TESTAPPL_
= { "Send an EoE ARP on all SDO access (upload/info/download) on 0x1018", 0, 0 }
#endif
;

/** \brief Test : Send an invalid mailbox data on all SDO access (upload/info/download) on 0x1018*/
PROTO TTESTENTRY SendInvalidMbxOn0x1018Access
#ifdef _TESTAPPL_
= { "Send an invalid mailbox data on all SDO access (upload/info/download) on 0x1018", 0, 0 }
#endif
;

/** \brief Test : Send an ARP request to the gateway when receiving an EoE Set parameter request (before) sending the Set parameter response*/
PROTO TTESTENTRY SendArpOnSetIPParamter
#ifdef _TESTAPPL_
= { "Send an ARP on Set IP Parameter (before sending the response)", 0, 0 }
#endif
;

/** \brief Test : Send an ARP request when receiving an unsupported mailbox protocol request (before sending the unsupported protocol response)*/
PROTO TTESTENTRY SendArpOnUnsupportedMailboxProtocolRequest
#ifdef _TESTAPPL_
= { "Send an ARP on an unsupported mailbox protocol request", 0, 0 }
#endif
;

/** \brief 0x2040 EoE Test object variable*/
PROTO TTESTOBJ sEoE0x2040
#ifdef _TESTAPPL_
= { 0x04, {&SendEoEOn0x1018Access,&SendInvalidMbxOn0x1018Access,&SendArpOnSetIPParamter,&SendArpOnUnsupportedMailboxProtocolRequest } }
#endif
;
#ifdef _OBJD_
/** \brief 0x2020 CoE Test object name*/
OBJCONST UCHAR OBJMEM aName0x2040[] = "EoE behaviour group 01\000\377";
#endif

/*ECATCHANGE_END(V5.13) TEST2*/

/*---------------------------------------------
-    Objects 0x20FF : Test control object
-----------------------------------------------*/
...
#if MEMORY_UNIT_16BIT
    {DEFTYPE_OCTETSTRING, (8*SIZEOF(sEnum0800_Value00) / 2), ACCESS_READ | OBJACCESS_NOPDOMAPPING},
   {DEFTYPE_OCTETSTRING, (8*SIZEOF(sEnum0800_Value01) / 2), ACCESS_READ | OBJACCESS_NOPDOMAPPING}};
#else
...
CHAR sEnum0802_Value00[] = "\310\000\000\000Two hundred"; /**< \brief Enum 0x802 value 0
...
/*ECATCHANGE_START(V5.13) */
#if MEMORY_UNIT_16BIT
{ DEFTYPE_OCTETSTRING, (8 * SIZEOF(sEnum0802_Value00) / 2), ACCESS_READ | OBJACCESS_NOPDOMAPPING },
{ DEFTYPE_OCTETSTRING, (8 * SIZEOF(sEnum0802_Value01) / 2), ACCESS_READ | OBJACCESS_NOPDOMAPPING } };
#else
...
#ifdef _TESTAPPL_
= {4, (REAL32) 2.34 ,(REAL64) 10.87 , 0xBABABABABABABA , 0xBABABABABABABA }
#endif
    ;
...
/** \brief 0x3018 (string object) entry descriptions*/
#ifdef _OBJD_
OBJCONST TSDOINFOENTRYDESC OBJMEM sEntryDesc0x3018 = { DEFTYPE_VISIBLESTRING,  240, ACCESS_READWRITE } ;

OBJCONST UCHAR OBJMEM aName0x3018[] =
"string object\000\377";
#endif //#ifdef _OBJD_

PROTO CHAR sObj3018_String[30]
#ifdef _TESTAPPL_
= "DummyData"
#endif
;

/** \brief 0x3019 (UINT16 object) entry descriptions*/
#ifdef _OBJD_
OBJCONST TSDOINFOENTRYDESC OBJMEM sEntryDesc0x3019 = { DEFTYPE_UNSIGNED16,  0x10, ACCESS_READWRITE };

OBJCONST UCHAR OBJMEM aName0x3019[] =
"Explicit Device ID\000\377";
#endif //#ifdef _OBJD_

PROTO UINT16 Obj3019
#ifdef _TESTAPPL_
= 5
#endif
;

/** \brief 0x301A (Trigger ESM transition test object) entry descriptions*/
#ifdef _OBJD_
OBJCONST TSDOINFOENTRYDESC OBJMEM asEntryDesc0x301A[] = {
    { DEFTYPE_UNSIGNED8,  0x8, ACCESS_READ },
    { DEFTYPE_UNSIGNED8, 0x08, ACCESS_READWRITE },
    { 0,  0x8, 0 },
    { DEFTYPE_UNSIGNED16, 0x10, ACCESS_READWRITE }
};
OBJCONST UCHAR OBJMEM aName0x301A[] =
"Trigger ESM transition test object\000"
"Requested AL Status\000"
"\000"
"Requested AL Status Code\000"
"\377";
#endif //#ifdef _OBJD_
...
TOBJ301A;
PROTO TOBJ301A sObj301A_TriggerEsm
#ifdef _TESTAPPL_
#if CONTROLLER_16BIT 
= { 3, 0,0}
#elif CONTROLLER_32BIT
...
/** \brief 0x301B (BitTypes) entry descriptions*/
#ifdef _OBJD_
OBJCONST TSDOINFOENTRYDESC OBJMEM asEntryDesc0x301B[] = {
    { DEFTYPE_UNSIGNED8,  0x8, ACCESS_READ },
    { 0x0038, 0x09, ACCESS_READWRITE }, //BIT9
    { 0,  0x7, 0 },
    { 0x0039, 0x0A, ACCESS_READWRITE }, //BIT10
    { 0,  0x6, 0 },
    { 0x003A, 0x0B, ACCESS_READWRITE }, //BIT11
    { 0,  0x5, 0 },
    { 0x003B, 0x0C, ACCESS_READWRITE }, //BIT12
    { 0,  0x4, 0 },
    { 0x003C, 0x0D, ACCESS_READWRITE }, //BIT13
    { 0,  0x3, 0 },
    { 0x003D, 0x0E, ACCESS_READWRITE }, //BIT14
    { 0,  0x2, 0 },
    { 0x003E, 0x0F, ACCESS_READWRITE }, //BIT15
    { 0,  0x1, 0 },
    { 0x003F, 0x10, ACCESS_READWRITE } //BIT16
};

OBJCONST UCHAR OBJMEM aName0x301B[] =
"BitTypes\000"
"Bit9\000"
"\000"
"Bit10\000"
...
PROTO TOBJ301B sObj301B_BitTypes
#ifdef _TESTAPPL_
= { 15,{0,0,0,0,0,0,0,0}}
#endif
;
...
PROTO TSLV_CONFIG aSlaveConfig[49]
#ifdef _TESTAPPL_
= {{"ESC_EEPROM_EMULATION",ESC_EEPROM_EMULATION},
    ...
    {"TEST_APPLICATION_REDUCED_MEMORY",TEST_APPLICATION_REDUCED_MEMORY},
    {"EL9800_APPLICATION",EL9800_APPLICATION},
    {"CiA402_SAMPLE_APPLICATION",CiA402_SAMPLE_APPLICATION},
...
OBJCONST TSDOINFOENTRYDESC    OBJMEM asEntryDesc0xA000[] = {
   ...
   {DEFTYPE_UNSIGNED16, 0x10, ACCESS_READ}, /*Subindex040 "u16SmSync0Value" (UINT16*/
   {DEFTYPE_UNSIGNED16, 0x10, ACCESS_READ} }; /*Subindex041 "u16SmSync0Counter" (UINT16*/
...
\000Sync1WdCounter\000Sync1WdValue\000LatchInputSync0Value\000u16SmSync0Value\000u16SmSync0Counter\000\377";
#endif //#ifdef _OBJD_
...
#ifdef _OBJD_
/** \brief 0xA001 (Process data function calls) entry descriptions*/
OBJCONST TSDOINFOENTRYDESC    OBJMEM asEntryDesc0xA001[] = {
   {DEFTYPE_UNSIGNED8, 0x8, ACCESS_READ },
   {DEFTYPE_UNSIGNED32, 0x20, ACCESS_READWRITE}, /* SubIndex001 "nPdoOutputMappingCalls" (UINT32)*/
   {DEFTYPE_UNSIGNED32, 0x20, ACCESS_READWRITE}, /* SubIndex002 "nApplApplicationCalls" (UINT32)*/
   {DEFTYPE_UNSIGNED32, 0x20, ACCESS_READWRITE} }; /* SubIndex003 "nPdoInputMappingCalls" (UINT32)*/

/** \brief 0xA001 (Esm Variables) object and entry names*/
OBJCONST UCHAR OBJMEM aName0xA001[] = "Process data function calls\000PDO OutputMapping\000APPL Application\000PDO InputMapping\000\377";
#endif //#ifdef _OBJD_


typedef struct OBJ_STRUCT_PACKED_START {
    UINT16 u16SubIndex0;
    UINT32 nPdoOutputMappingCalls;
    UINT32 nApplApplicationCalls;
    UINT32 nPdoInputMappingCalls;
} OBJ_STRUCT_PACKED_END
TOBJA001;


PROTO TOBJA001 sObjA001_PdFncCallc
#ifdef _TESTAPPL_
= { 3,0,0,0 }
#endif
;
...
#ifdef _OBJD_
/** \brief 0xA002 (Function calls) entry descriptions*/
OBJCONST TSDOINFOENTRYDESC    OBJMEM asEntryDesc0xA002[] = {
   {DEFTYPE_UNSIGNED8, 0x8, ACCESS_READ },
   {DEFTYPE_UNSIGNED32, 0x20, ACCESS_READWRITE} }; /* SubIndex001 "nEcatCheckTimer" (UINT32)*/

/** \brief 0xA001 (Esm Variables) object and entry names*/
OBJCONST UCHAR OBJMEM aName0xA002[] = "Function calls\000EcatCheckTimer\000\377";
#endif //#ifdef _OBJD_


typedef struct OBJ_STRUCT_PACKED_START {
    UINT16 u16SubIndex0;
    UINT32 nEcatCheckTimer;
} OBJ_STRUCT_PACKED_END
TOBJA002;


PROTO TOBJA002 sObjA002_FncCallc
#ifdef _TESTAPPL_
= {1,0 }
#endif
;
...
PROTO UINT8 WriteTriggerEsmObject(UINT16 index, UINT8 subindex, UINT32 dataSize, UINT16 MBXMEM * pData, UINT8 bCompleteAccess);
...
TOBJECT    OBJMEM ApplicationObjDic[] = {
    ...
    /* Object 0x2000 */
    {NULL,NULL,  0x2000, {DEFTYPE_RECORD, 6 | (OBJCODE_REC << 8)}, asTestObjEntryDesc, aName0x2000, &sESM0x2000, ReadTestObject, WriteTestObject, 0x0000 },
    /* Object 0x2010 */
    {NULL,NULL,  0x2010, {DEFTYPE_RECORD, 4 | (OBJCODE_REC << 8)}, asTestObjEntryDesc, aName0x2010, &sMbx0x2010, ReadTestObject, WriteTestObject, 0x0000 },
    /* Object 0x2020 */
    ...
    /*ECATCHANGE_START(V5.13) TEST2*/
    /* Object 0x2040 */
    {NULL,NULL,  0x2040, {DEFTYPE_RECORD, 4 | (OBJCODE_REC << 8)}, asTestObjEntryDesc, aName0x2040, &sEoE0x2040, ReadTestObject, WriteTestObject, 0x0000 },
    /*ECATCHANGE_END(V5.13) TEST2*/
    /* Object 0x2FFF */
...
#if SEGMENTED_SDO_SUPPORTED
    /* Object 0x3009*/
    {NULL,NULL,  0x3009, {DEFTYPE_OCTETSTRING, 255 | (OBJCODE_ARR << 8)}, asEntryDesc0x3009, aName0x3009, &sObj3009, ReadObject0x3009_0x300A, WriteObject0x3009_0x300A, 0x0000 },    
    /* Object 0x300A*/
    {NULL,NULL,  0x300A, { DEFTYPE_RECORD, 255 | (OBJCODE_ARR/*will be set to REC in SDO Info*/ << 8)}, asEntryDesc0x3009, aName0x300A, &sObj3009, ReadObject0x3009_0x300A, WriteObject0x3009_0x300A, 0x0000 },
#endif //#if SEGMENTED_SDO_SUPPORTED
    /* Object 0x300B*/
...
#endif
    /* Object 0x3018 */
    { NULL,NULL,  0x3018,{ DEFTYPE_VISIBLESTRING, 0 | (OBJCODE_VAR << 8) }, &sEntryDesc0x3018, aName0x3018, &sObj3018_String, NULL, NULL, 0x0000 },
    /* Object 0x3019 */
    { NULL,NULL,  0x3019,{ DEFTYPE_UNSIGNED16, 0 | (OBJCODE_VAR << 8) }, &sEntryDesc0x3019, aName0x3019, &Obj3019, NULL, NULL, 0x0000 },
    /* Object 0x301A */
    { NULL,NULL,  0x301A,{ DEFTYPE_RECORD, 3 | (OBJCODE_REC << 8) }, asEntryDesc0x301A, aName0x301A, &sObj301A_TriggerEsm, NULL, WriteTriggerEsmObject, 0x0000 },
        /* Object 0x301B */
    { NULL,NULL,  0x301B,{ DEFTYPE_RECORD, 15 | (OBJCODE_REC << 8) }, asEntryDesc0x301B, aName0x301B, &sObj301B_BitTypes, NULL, 0x0000, 0x0000 },
    /* Object 0x6000 */
...
#if DC_SUPPORTED
    /* Object 0xA000 */
    {NULL,NULL,   0xA000, {DEFTYPE_RECORD, 41 | (OBJCODE_REC << 8)}, asEntryDesc0xA000, aName0xA000, &A000Subindex0, ReadObject0xA000, NULL, 0x0000 },
#endif
/*ECATCHANGE_START(V5.13) TEST5*/
    /* Object 0xA001 */
    { NULL,NULL,   0xA001, {DEFTYPE_RECORD, 3 | (OBJCODE_REC << 8)}, asEntryDesc0xA001, aName0xA001, &sObjA001_PdFncCallc, NULL, NULL, 0x0000 },
/*ECATCHANGE_START(V5.13) TEST6*/
        /* Object 0xA002 */
    { NULL,NULL,   0xA002, {DEFTYPE_RECORD, 1 | (OBJCODE_REC << 8)}, asEntryDesc0xA002, aName0xA002, &sObjA002_FncCallc, NULL, NULL, 0x0000 },
/*ECATCHANGE_END(V5.13) TEST6*/
/*ECATCHANGE_END(V5.13) TEST5*/
    /* Object 0xF000 */
...
PROTO void HandleCttTestCaseID(void);
...
PROTO UINT16 u16PsTransitionCallsHelper; /**< \brief helper variable to check the number of PS transition calls, will be reset in Init (only incremented if "ESM0_PENDING_PS_TRANSITION_WITH_ERROR" is enabled)*/


PROTO UINT16 u16CurrentTestID; /**< \brief contains the current CTT Test ID (is written by some test cases to active a specific behavior)*/
PROTO UINT16 u16CurrentTestGroupID; /**< \brief contains the current CTT Test Group ID (is written by some test cases to active a specific behavior)*/
PROTO UINT16 u16CurrentTestCaseID; /**< \brief contains the current CTT Test Case ID (is written by some test cases to active a specific behavior)*/
#undef PROTO
/** @}*/
