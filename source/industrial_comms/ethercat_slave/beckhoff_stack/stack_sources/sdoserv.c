...
#if TEST_APPLICATION && EOE_SUPPORTED
...
static UINT8 SdoUploadSegmentInd(TUPLOADSDOSEGREQMBX MBXMEM * pSdoInd)
{
	...
	else
	{
		...
		{
			maxData = 16 - MBX_HEADER_SIZE - SEGMENT_NORM_HEADER_SIZE;
			INC_TEST_CNT(CoE0_SegSdoByte);
		}
		else
#endif
		{
...
#if BIG_ENDIAN_16BIT
		((UINT16 MBXMEM *) &((TABORTSDOTRANSFERREQMBX MBXMEM *) pSdoRes)->AbortCode)[0] = ((UINT16 *)&cAbortCode[abort])[1];
		((UINT16 MBXMEM *) &((TABORTSDOTRANSFERREQMBX MBXMEM *) pSdoRes)->AbortCode)[1] = ((UINT16 *)&cAbortCode[abort])[0];
#else
...
#if TEST_APPLICATION && EOE_SUPPORTED
		if (index == 0x1018 && IS_TEST_ACTIVE(SendEoEOn0x1018Access))
		{
			UINT16 result = 0;
			...
			pArpIp->ipDest[0] = 0xBABA;
			pArpIp->ipDest[1] = 0xBABA;
			...
			if (result == 0)
			{
				INC_TEST_CNT(SendEoEOn0x1018Access);
			}
...
#if TEST_APPLICATION
			if (index == 0x1018 && IS_TEST_ACTIVE(SendInvalidMbxOn0x1018Access))
			{
				UINT16 result = 0;
				...
				result = MBX_MailboxSendReq(pData, 0xEF);

				if (result == 0)
				{
					INC_TEST_CNT(SendInvalidMbxOn0x1018Access);
				}
			}
		...
		if (SWAPWORD(pSdoInfoInd->SdoHeader.Data.List.ListType) <= INFO_LIST_TYPE_MAX)
		{
			...
			else
			{
				...
				if (nSdoInfoFragmentsLeft)
				{
					...
					if (IS_TEST_ACTIVE(CoE0_SegSdoByte))
					{
						size = 16 - SIZEOF_SDOINFO - MBX_HEADER_SIZE;
						INC_TEST_CNT(CoE0_SegSdoByte);
					}
					else
#endif
...
#endif
				{
					...
					if (IS_TEST_ACTIVE(CoE0_SegSdoByte))
					{
						size = 16 - SIZEOF_SDOINFOLISTSTRUCT - MBX_HEADER_SIZE;
						INC_TEST_CNT(CoE0_SegSdoByte);
					}
					else
#endif
		...
		else
		{
			...
			if (pObjEntry)
			{
				...
				if (opCode == SDOINFOSERVICE_OBJDESCRIPTION_Q)
				{
					...
					pSdoInfoInd->SdoHeader.Data.Obj.Res.DataType = SWAPWORD(pObjDesc->DataType);
					pSdoInfoInd->SdoHeader.Data.Obj.Res.ObjFlags = SWAPWORD(pObjDesc->ObjFlags);
#else
...
#if TEST_APPLICATION
					/*change object code for object 0x300A from ARRARY to RECORD*/
					if (pObjEntry->Index == 0x300A)
					{
						...
						pSdoInfoInd->SdoHeader.Data.Obj.Res.ObjFlags = SWAPWORD(Flags);
					}
#endif
				...
				else
				{
					...
					if (subindex <= maxSubindex)
					{
						...
						pSdoInfoInd->SdoHeader.Data.Entry.Res.DataType = SWAPWORD(pEntryDesc->DataType);
						pSdoInfoInd->SdoHeader.Data.Entry.Res.BitLength = SWAPWORD(pEntryDesc->BitLength);
						pSdoInfoInd->SdoHeader.Data.Entry.Res.ObjAccess = SWAPWORD(pEntryDesc->ObjAccess);
#else
